...
(async () => {
  ...
  try {
    ...
    // Evaluate more aggressively to remove the overlay AND the background overlay
    await page.evaluate(() => {
        const dialogs = document.querySelectorAll('div');
        for (const d of dialogs) {
            // Find modals and their backdrops and remove them
            if (d.className.includes('fixed inset-0 bg-black/50') || d.className.includes('z-50')) {
                d.remove();
            }
        }
    });
    ...
    await page.screenshot({ path: 'verification_admin_quick_notes_final_3.png' });
  } catch (error) {
