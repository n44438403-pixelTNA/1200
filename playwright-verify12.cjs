...
(async () => {
  ...
  await page.evaluate(() => {
    ...
    const dismiss = buttons.find(b => b.textContent && b.textContent.includes('Dismiss'));
  ...
  await page.evaluate(() => {
    ...
    const quickNotes = headings.find(h => h.textContent && h.textContent.includes('Quick Notes Breakdown'));
    if (quickNotes) {
  ...
  await page.screenshot({ path: 'verification_admin_quick_notes_final_9.png' });
  await browser.close();
  console.log('Screenshot saved to verification_admin_quick_notes_final_9.png');
})();
