...
(async () => {
  ...
  try {
    ...
    await page.evaluate(() => {
        ...
        const headers = Array.from(document.querySelectorAll('div, span, h3')).filter(h => h.textContent && h.textContent.includes('CONTENT & ANALYSIS'));
        if (headers.length > 0) {
           headers[0].click();
        }
    });
    ...
    await page.evaluate(() => {
        const buttons = Array.from(document.querySelectorAll('button, div')).filter(b => b.textContent && b.textContent.includes('Content Manager'));
    ...
    await page.screenshot({ path: 'verification_admin_quick_notes_final_5.png' });
  } catch (error) {
