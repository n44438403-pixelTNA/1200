...
const App: React.FC = () => {
  ...
  useEffect(() => {
    storage.getItem<StudentTab>('nst_active_student_tab').then(saved => {
        ...
        if (saved && !transientTabs.includes(saved as string)) {
            setStudentTab(saved);
        } else if (saved) {
            setStudentTab('HOME');
        }
  ...
  useEffect(() => {
    ...
    const hydrateUser = async () => {
      if (loggedInUserStr) {
        try {
          let lightUser: User = JSON.parse(loggedInUserStr);

          // STRICT VALIDATION: Ensure critical fields exist
          if (!lightUser || !lightUser.id || !lightUser.role) {
              console.error("Invalid user object found in storage. Clearing session.");
              localStorage.removeItem('nst_current_user');
              return;
          }

          // Fetch heavy data from IndexedDB
          const bulkyData = await storage.getItem("nst_user_bulky") as Partial<User>;
          let user: User = { ...lightUser, ...(bulkyData || {}) };
          ...
          if (user.role !== 'ADMIN') {
               ...
               if (JSON.stringify(lightUser) !== loggedInUserStr) { // changed slightly to be cleaner
                   saveUserLocal(user);
    ...
    if (state.user) {
        interval = setInterval(() => {
            setDailyStudySeconds(prev => {
                ...
                if (next % 60 === 0) updateUserStatus(state.user!.id, next);
                return next;
            });
        }, 1000);
    }
  ...
  return (
    <ErrorBoundary>
    <div className={`min-h-screen flex flex-col bg-slate-50 font-sans relative pt-[env(safe-area-inset-top,24px)] pb-[env(safe-area-inset-bottom,32px)] ${appState.user?.role === 'TEACHER' ? 'app-role-teacher' : 'app-role-student'}`}>
